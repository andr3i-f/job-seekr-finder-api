...
# TODO: Implement redis if necessary; this is caching per worker in prod which isn't ideal but it's not the end of the world.
    ...
    if (
        ...
        try:
            async with httpx.AsyncClient() as client:
                res = await client.get(JWKS_URL, timeout=5)
...
async def verify_jwt_token(token: str) -> JWTTokenPayload:
